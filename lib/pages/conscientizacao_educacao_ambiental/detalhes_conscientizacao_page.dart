...
class DetalhesConscientizacaoPage extends StatelessWidget {
  final Registro registro;

  const DetalhesConscientizacaoPage({super.key, required this.registro});
  ...
  String _formatarAcao(String? acaoStr, String? outro) {
    if (acaoStr == null || acaoStr.isEmpty) return '--';
    
    if (acaoStr == 'OUTRO' && outro != null && outro.isNotEmpty) {
      return 'Outro: $outro';
    }

    try {
      final acaoEnum = TipoAcaoConscientizacao.values.firstWhere(
        (e) => e.name == acaoStr
      );
      
      return acaoEnum.descricao;
    } catch (_) {
      return acaoStr.replaceAll('_', ' '); 
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('RO Conscientização: ${registro.id}'),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDespachoConscientizacaoSection(context),
            const SizedBox(height: 20),
  ...
  Widget _buildDespachoConscientizacaoSection(BuildContext context) {
    return Card(
      elevation: 2,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow('Ação Prevista', _formatarAcao(registro.tipoAcaoConscientizacao, registro.acaoOutroConscientizacao)),
            _buildInfoRow('Projetos Sentinelas', registro.projetosSentinelas == true ? 'Sim' : 'Não'),
            const SizedBox(height: 12),
            ...
            if (registro.arquivosDespachoConscientizacao.isNotEmpty)
              _buildGaleriaImagens(registro.arquivosDespachoConscientizacao),
          ],
        ),
      ),
    );
